...
export class HomePageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.subscriptions.push(
      this.carsService.getAllCars({ liked: true }).subscribe((cars) => {
        this.likedCars = cars;
        this.loading = false;
      }),
    );
    this.subscriptions.push(
      this.carsService.getAllCars({ newItem: true }).subscribe((cars) => {
    ...
    this.subscriptions.push(
      this.dealersService
        .getAllDealers({ newRecord: true })
        .subscribe((dealer) => {
          this.newlyAddedDealers = dealer;
          this.loading = false;
        }),
    );
  }
